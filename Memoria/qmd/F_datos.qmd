...
cargar_conjunto <- function(fichero, columna_etiqueta, base, ventana) {
  marco <- read.csv(file.path(ruta_datos, fichero), stringsAsFactors = FALSE, check.names = FALSE)
  names(marco)[names(marco) == columna_etiqueta] <- "etiqueta"
  marco |>
    mutate(
      sexo    = case_when(gender %in% c("M", "Male")   ~ "Hombre",
...
mimic_corta  <- cargar_conjunto("definitivo_v4p.csv",        "etiqueta_norad_3_12",  "MIMIC-IV", "3 h")
mimic_media  <- cargar_conjunto("definitivo_v4.csv",         "etiqueta_norad_6_24",  "MIMIC-IV", "6 h")
mimic_larga  <- cargar_conjunto("definitivo_v4l.csv",        "etiqueta_norad_12_48", "MIMIC-IV", "12 h")
eicu_corta  <- cargar_conjunto("eICU_3_12_definitivo.csv",  "etiqueta_norad_3_12",  "eICU-CRD", "3 h")
eicu_media  <- cargar_conjunto("eICU_6_24_definitivo.csv",  "etiqueta_norad_6_24",  "eICU-CRD", "6 h")
eicu_larga  <- cargar_conjunto("eICU_12_48_definitivo.csv", "etiqueta_norad_12_48", "eICU-CRD", "12 h")
```

La @tbl-comparacion-cohortes resume las características basales de las seis
cohortes. Las dos bases presentan una edad y una distribución por sexo
similares (en torno a 60 años, con leve predominio masculino).

```{r}
#| label: tbl-comparacion-cohortes
#| tbl-cap: "Características de las cohortes de desarrollo (MIMIC-IV) y de validación externa (eICU-CRD) en las tres ventanas temporales."
...
resumir_cohorte <- function(marco) {
  ...
  tibble(
    ...
    Peso          = sprintf("%.1f \u00b1 %.1f", mean(marco$peso_kg, na.rm = TRUE), sd(marco$peso_kg, na.rm = TRUE)),
...
kable(tabla_cohortes, booktabs = TRUE, format = "latex", align = "lrrrrrr",
      col.names = c("Conjunto", "N", "Edad (a\u00f1os)", "Hombres", "Peso (kg)",
                    "Noradrenalina", "SOFA m\u00e1x"))
```

El contraste se confirma al examinar los predictores compartidos por ambas bases en la ventana media (@tbl-predictores-comparados): eICU-CRD presenta
una PAM mínima más baja y un SOFA máximo más alto, mientras que el cociente PaO2/FiO2 mínimo es mayor; en conjunto, un perfil hemodinámico más comprometido
pero con mejor oxigenación basal. Estas diferencias no invalidan la validación externa, sino que delimitan el grado de extrapolación exigido a los modelos.

```{r}
#| label: tbl-predictores-comparados
#| tbl-cap: "Distribución de las variables predictoras compartidas por MIMIC-IV y eICU-CRD en la ventana media (observación 0--6 h). Mediana [P25--P75]."

variables_compartidas <- c("map_min", "pf_min", "sofa_max", "hr_media", "rr_max", "glucemia_min", "diuresis_ml_kg_6h")
etiquetas_compartidas <- c("PAM m\u00edn (mmHg)", "PaO2/FiO2 m\u00edn", "SOFA m\u00e1x","FC media (lpm)", "FR m\u00e1x (rpm)", "Glucemia m\u00edn (mg/dL)", "Diuresis (mL/kg/6 h)")

resumir_variable <- function(x) {sprintf("%.1f [%.1f\u2013%.1f]", median(x, na.rm = TRUE), quantile(x, 0.25, na.rm = TRUE), quantile(x, 0.75, na.rm = TRUE))
}
...
kable(tabla_predictores, booktabs = TRUE, format = "latex", align = "lcc",
      col.names = c("Variable",
                    paste0("MIMIC-IV (n = ", nrow(mimic_media), ")"),
                    paste0("eICU-CRD (n = ", nrow(eicu_media), ")")))
```

Las distribuciones de la @fig-densidades-comparadas hacen visible ese desplazamiento de dominio: las densidades de eICU-CRD aparecen desplazadas
respecto a las de MIMIC-IV en la mayoría de los predictores, de forma más acusada en el SOFA máximo y en el cociente PaO2/FiO2 mínimo.
...
variables_densidad <- c("map_min", "pf_min", "hr_media", "rr_max", "glucemia_min", "sofa_max")
etiquetas_densidad <- c("PAM m\u00edn (mmHg)", "PaO2/FiO2 m\u00edn", "FC media (lpm)",
                       "FR m\u00e1x (rpm)", "Glucemia m\u00edn (mg/dL)", "SOFA m\u00e1x")
names(etiquetas_densidad) <- variables_densidad
...
datos_densidad <- bind_rows(
  ...
  pivot_longer(all_of(variables_densidad), names_to = "variable", values_to = "valor") |>
  mutate(variable = factor(variable, levels = variables_densidad,
                           labels = unname(etiquetas_densidad[variables_densidad])))

ggplot(datos_densidad, aes(x = valor, fill = base)) +
  geom_density(alpha = 0.55, color = NA) +
  facet_wrap(~ variable, scales = "free", ncol = 3) +
  scale_fill_manual(values = c("MIMIC-IV" = "#3B6EA5", "eICU-CRD" = "#E07B39")) +
  labs(x = NULL, y = "Densidad", fill = "Base de datos") +
  theme_minimal(base_size = 10) +
  theme(legend.position = "bottom")
```

Pese a esas diferencias de partida, la dirección de la asociación con el desenlace se conserva entre cohortes (@fig-predictores-desenlace): tanto en
MIMIC-IV como en eICU-CRD, los pacientes que recibieron noradrenalina presentan una PAM mínima y un cociente PaO2/FiO2 mínimo inferiores y un SOFA
máximo superior. Esta coherencia en el sentido del efecto es lo que hace plausible la transferencia de los modelos a la cohorte externa.
...
etiquetas_desenlace <- c("PAM m\u00edn (mmHg)", "PaO2/FiO2 m\u00edn", "SOFA m\u00e1x")
names(etiquetas_desenlace) <- variables_desenlace

datos_desenlace <- bind_rows(
  mimic_media |> select(base, etiqueta, all_of(variables_desenlace)),
  eicu_media  |> select(base, etiqueta, all_of(variables_desenlace))
) |>
  pivot_longer(all_of(variables_desenlace), names_to = "variable", values_to = "valor") |>
  mutate(
    variable  = factor(variable, levels = variables_desenlace,
                       labels = unname(etiquetas_desenlace[variables_desenlace])),
    desenlace = factor(etiqueta, levels = c(0, 1),
                       labels = c("No noradrenalina", "Noradrenalina"))
  )

ggplot(datos_desenlace, aes(x = base, y = valor, fill = desenlace)) +
  geom_boxplot(outlier.size = 0.4, alpha = 0.85, width = 0.6) +
  facet_wrap(~ variable, scales = "free_y") +
  scale_fill_manual(values = c("No noradrenalina" = "#7FB069", "Noradrenalina" = "#D1495B")) +
  labs(x = NULL, y = "Valor", fill = "Desenlace") +
  theme_minimal(base_size = 10) +
  theme(legend.position = "bottom")
```

Por último, el tiempo desde el inicio de la ventana de observación hasta la primera dosis de noradrenalina (@fig-tiempo-norad) escala con el horizonte de
predicción —mediana en torno a 6, 11 y 20 h para las ventanas corta, media y larga, respectivamente— y resulta muy similar entre MIMIC-IV y eICU-CRD. Los
valores respetan en todos los casos los límites inferiores de cada horizonte, lo que confirma la consistencia del etiquetado entre ambas bases y la ausencia
...
datos_tiempo <- bind_rows(
  seleccionar_tiempo(mimic_corta), seleccionar_tiempo(mimic_media), seleccionar_tiempo(mimic_larga),
  seleccionar_tiempo(eicu_corta),  seleccionar_tiempo(eicu_media),  seleccionar_tiempo(eicu_larga)
) |>
  filter(etiqueta == 1, !is.na(horas_hasta_norad)) |>
  mutate(ventana = factor(ventana, levels = c("3 h", "6 h", "12 h"),
                          labels = c("Ventana corta (3\u201312 h)",
                                     "Ventana media (6\u201324 h)",
                                     "Ventana larga (12\u201348 h)")))

ggplot(datos_tiempo, aes(x = horas_hasta_norad, fill = base)) +
  geom_histogram(bins = 25, position = "identity", alpha = 0.6, color = "white") +
  facet_wrap(~ ventana, scales = "free", ncol = 3) +
  scale_fill_manual(values = c("MIMIC-IV" = "#3B6EA5", "eICU-CRD" = "#E07B39")) +
  labs(x = "Horas hasta noradrenalina", y = "N\u00famero de pacientes", fill = "Base de datos") +
  theme_minimal(base_size = 10) +
  theme(legend.position = "bottom")
```