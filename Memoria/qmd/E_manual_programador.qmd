...
aplicación Streamlit. Deben distinguirse dos niveles de ejecución: el uso del \textit{dashboard} y la reproducción del flujo experimental.

\subsection*{Ejecución del \textit{dashboard}}

Para ejecutar únicamente el prototipo de visualización no es necesario descargar MIMIC-IV ni eICU-CRD. Basta con disponer de la carpeta \ruta{Dashboard/}, que contiene
el script de Streamlit, los modelos serializados y las referencias de percentiles.
...
\end{verbatim}
  ...
  \item Crear un entorno virtual (recomendado):

\begin{verbatim}
python -m venv .venv
\end{verbatim}

  \item Activar el entorno virtual. En Windows (recomendado):
...
\end{verbatim}
  ...
  \item Instalar las dependencias principales:
...
\end{verbatim}
  ...
  \item Abrir la dirección indicada por Streamlit en el navegador. Habitualmente será:
...
Para que la aplicación funcione correctamente, deben permanecer en la carpeta \ruta{Dashboard/} los siguientes archivos:

\begin{itemize}
  \item \ruta{pruebas\_dashboard.py};
  \item \ruta{modelo\_Medio\_6\_24\_XGB\_calibrado.pkl};
  \item \ruta{modelo\_Largo\_12\_48\_XGB.pkl};
  \item \ruta{referencia\_percentiles\_medio.npy};
  \item \ruta{referencia\_percentiles\_largo.npy}.
\end{itemize}

Si alguno de estos ficheros falta, cambia de nombre o se desplaza a otra ubicación, el \textit{dashboard} puede fallar al cargar el modelo o al calcular el percentil de riesgo.
...
requerirían rediseñar parcialmente la representación de los datos, ya que el flujo actual se basa en estadísticos agregados por ventana temporal.
