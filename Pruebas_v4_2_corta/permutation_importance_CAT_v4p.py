...
def run_cv(predictores, etiqueta, paciente_id):
    ...
    for nf, (idx_tr, idx_te) in enumerate(
        ...
        imp_std[nf-1, :]    = res.importances_std
...
def graficar(df_res, ruta, auc_m):
    ...
    ax.set_title(f'Permutation Importance — CatBoost v4p CORTA (26 vars)\n'
...
def main():
    ...
    ruta_tab = os.path.join(CARPETA_TABLAS,  'permutation_importance_CAT_v4p.csv')
    ruta_fig = os.path.join(CARPETA_FIGURAS, 'permutation_importance_CAT_v4p.png')
