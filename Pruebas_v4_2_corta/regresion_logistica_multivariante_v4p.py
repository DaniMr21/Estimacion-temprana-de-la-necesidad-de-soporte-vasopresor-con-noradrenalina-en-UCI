...
if os.path.exists(ruta_uni):
    ...
    comp.to_csv(os.path.join(CARPETA_TABLAS,
                'comparacion_univariante_vs_multivariante_v4p.csv'), index=False)
else:
    print(f"  No encontrado: {ruta_uni}. Ejecuta primero significancia_univariante_v4p.py")

tabla_vars.to_csv(os.path.join(CARPETA_TABLAS,
                 'regresion_logistica_multivariante_v4p.csv'), index=False)
print(f"\nTabla guardada en: tablas/regresion_logistica_multivariante_v4p.csv")
