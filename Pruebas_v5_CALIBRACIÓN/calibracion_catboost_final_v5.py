"""
CALIBRACION FINAL — CatBoost global — v5

Ajustado a los modelos finales del TFG:
  - CatBoost global en las tres ventanas.
  - Variables finales reducidas usadas en baselines_con_metricas_v5*.py.
  - Validacion externa StratifiedGroupKFold agrupada por subject_id.
  - Calibracion dentro del entrenamiento externo, sin tocar el fold test.

Salidas:
  calibracion_catboost_final_v5/
    tablas/
      predicciones_calibracion_<ventana>.csv
      metricas_calibracion_<ventana>.csv
      resumen_calibracion_<ventana>.csv
      mejores_parametros_<ventana>.csv
      seleccion_calibrador_<ventana>.csv
      tabla_calibracion_deciles_<ventana>.csv
      metricas_calibracion_todas_las_ventanas.csv
      resumen_calibracion_todas_las_ventanas.csv
      sensibilidad_ece_<ventana>.csv
      sensibilidad_ece_todas_las_ventanas.csv
    figuras/
      calibracion_<ventana>.png
      histograma_probabilidades_<ventana>.png
      roc_<ventana>.png
      precision_recall_<ventana>.png

Nota metodologica:
  Para calibrar, cada fold externo se divide internamente a nivel de paciente en:
    - subconjunto modelo: GridSearchCV + entrenamiento CatBoost
    - subconjunto calibracion: ajuste sigmoid/isotonic
    - subconjunto test externo: evaluacion final

  Por tanto, estas metricas de calibracion no tienen por que ser identicas a las
  metricas preliminares entrenadas con todo el entrenamiento externo, porque aqui
  se reserva una parte para calibrar.
"""
