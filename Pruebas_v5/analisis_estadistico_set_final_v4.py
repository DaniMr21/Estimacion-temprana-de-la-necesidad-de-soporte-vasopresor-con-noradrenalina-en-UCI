"""
Análisis estadístico completo — v4 SET FINAL (15 variables).
Ventana: 0-6h observación / 6-24h predicción.

Ejecuta en secuencia:
  1. Significancia univariante (Mann-Whitney / Chi-cuadrado + BH-FDR)
  2. Regresión logística multivariante (statsmodels + BH-FDR)
  3. Permutation importance sobre RF (CV anidado, 50 permutaciones)

Salidas:
  - tablas/univariante_set_final_v4.csv
  - tablas/multivariante_set_final_v4.csv
  - tablas/permutation_importance_set_final_v4.csv
  - figuras/permutation_importance_set_final_v4.png
"""
...
VARIABLES = [
    ...
    'glucemia_min', 'temp_min', 'tp_max'
]
VARS_BINARIAS  = []  # ninguna binaria en este set
VARS_CONTINUAS = VARIABLES.copy()
...
# ── CARGA ─────────────────────────────────────────────────────────────────────
print("=" * 65)
print("ANÁLISIS ESTADÍSTICO SET FINAL — v4 (15 variables, 6-24h)")
print("=" * 65)
...
# Para PI: dataset completo (como en el baseline)
...
uni.to_csv(os.path.join(CARPETA_TABLAS,'univariante_set_final_v4.csv'), index=False)
...
multi.to_csv(os.path.join(CARPETA_TABLAS,'multivariante_set_final_v4.csv'), index=False)
...
# ═══════════════════════════════════════════════════════════════════════════════
print("\n" + "─"*65)
print("[3/3] PERMUTATION IMPORTANCE — RF")
print("─"*65)

pipeline_pi = Pipeline([('modelo', RandomForestClassifier(random_state=RS, n_jobs=-1))])
espacio_pi  = {
...
print(f"\n  AUC medio RF (15 vars): {auc_pi:.4f} ± {std_pi:.4f}  "
...
ax.set_title(f'Permutation Importance — RF SET FINAL v4 (15 vars)\n'
...
plt.tight_layout()
ruta_fig = os.path.join(CARPETA_FIGS, 'permutation_importance_set_final_v4.png')
plt.savefig(ruta_fig, dpi=150, bbox_inches='tight'); plt.close()

pi.to_csv(os.path.join(CARPETA_TABLAS,'permutation_importance_set_final_v4.csv'), index=False)
print(f"\n  Figura: {ruta_fig}")
print("\n" + "="*65)
print("FIN — v4")
print("="*65)
