-- =============================================================================
-- DATASET VALIDACIÓN EXTERNA eICU — ventana 3-12h (equivalente a v4p de MIMIC)
-- =============================================================================

-- 1. DATASET MODELO v4p
...
-- 2. LABORATORIO (ventana 0-3h) — solo lo que usa el modelo v4p + auxiliares SOFA
drop table if exists variables_lab_eicu_v4p;
create table variables_lab_eicu_v4p as select c.patientunitstayid,
    ...
    -- Auxiliares para SOFA (no van al SELECT final)
...
from dataset_modelo_eicu_v4p c left join "lab" l on c.patientunitstayid = l.patientunitstayid
    and l.labresult is not null and l.labresultoffset between 0 and 180 and l.labname in ('lactate','pH','paO2', 'total bilirubin','platelets x 1000','creatinine')
group by c.patientunitstayid;


-- 3. VITALES (ventana 0-3h)
drop table if exists variables_vitales_eicu_v4p;
create table variables_vitales_eicu_v4p as with periodic as (select c.patientunitstayid,
        avg(vp.heartrate)    as hr_media,
        max(vp.respiration)  as rr_max,
        min(vp.sao2)         as spo2_min,
        min(vp.systemicmean) as map_art_min,
        min(case when vp.temperature > 50 then (vp.temperature - 32) * 5.0/9.0 else vp.temperature end) as temp_min_periodic
    from dataset_modelo_eicu_v4p c
    left join "vitalPeriodic" vp on c.patientunitstayid = vp.patientunitstayid and vp.observationoffset between 0 and 180
    group by c.patientunitstayid), aperiodic as (select c.patientunitstayid, min(va.noninvasivemean) as map_nibp_min
	
    from dataset_modelo_eicu_v4p c left join "vitalAperiodic" va on c.patientunitstayid = va.patientunitstayid and va.observationoffset between 0 and 180
    group by c.patientunitstayid), temp_nurse as (select c.patientunitstayid,
        min(case
    ...
    from dataset_modelo_eicu_v4p c left join "nurseCharting" nc on c.patientunitstayid = nc.patientunitstayid and nc.nursingchartoffset between 0 and 180
...
-- 4. FiO2 NORMALIZADO (ventana 0-3h) — solo para pf_min
...
with raw as (
    ...
    from dataset_modelo_eicu_v4p c left join "respiratoryCharting" rc on c.patientunitstayid = rc.patientunitstayid
...
-- 5. GCS (ventana 0-3h) — solo para SOFA, no se expone
...
from dataset_modelo_eicu_v4p c
left join "nurseCharting" nc on c.patientunitstayid = nc.patientunitstayid and nc.nursingchartoffset between 0 and 180 and nc.nursingchartcelltypevalname = 'GCS Total'
group by c.patientunitstayid;


-- 6. DIURESIS normalizada por peso (ventana 0-3h)
drop table if exists variables_diuresis_eicu_v4p;
create table variables_diuresis_eicu_v4p as
select c.patientunitstayid,
    case
...
from dataset_modelo_eicu_v4p c left join "intakeOutput" io on c.patientunitstayid = io.patientunitstayid and io.intakeoutputoffset between 0 and 180
group by c.patientunitstayid, c.peso_kg;


-- 7. SOFA (ventana 0-3h)
drop table if exists variables_sofa_eicu_v4p;
create table variables_sofa_eicu_v4p as with vasopresores as (select
        c.patientunitstayid,
    ...
    from dataset_modelo_eicu_v4p c left join "infusionDrug" id on c.patientunitstayid = id.patientunitstayid
        and id.infusionoffset between 0 and 180 and lower(id.drugname) not like '%volume%'
    group by c.patientunitstayid),
...
-- 8. TABLA FINAL — solo las 10 variables del modelo + identificadores
drop table if exists dataset_final_eicu_v4p;
create table dataset_final_eicu_v4p as select
    c.uniquepid         as subject_id,
    c.patientunitstayid as stay_id,
    c.anchor_age,c.gender,c.peso_kg,c.contador_estancia_uci,c.horas_hasta_norad,c.etiqueta_norad_3_12,
	-- 10 variables del modelo v4p
    vt.map_min, vt.hr_media,
    case
        when l.pao2_min is not null and f.fio2_max is not null and f.fio2_max > 0
        then l.pao2_min / (f.fio2_max / 100.0)
        else null
    end as pf_min,vt.spo2_min,vt.rr_max,d.diuresis_ml_kg_3h,l.lactato_max,l.ph_min,vt.temp_min,so.sofa_max
from dataset_modelo_eicu_v4p c
left join variables_lab_eicu_v4p      l  on c.patientunitstayid = l.patientunitstayid
left join variables_vitales_eicu_v4p  vt on c.patientunitstayid = vt.patientunitstayid
left join variables_fio2_eicu_v4p     f  on c.patientunitstayid = f.patientunitstayid
left join variables_diuresis_eicu_v4p d  on c.patientunitstayid = d.patientunitstayid
left join variables_sofa_eicu_v4p     so on c.patientunitstayid = so.patientunitstayid;


-- 9. TABLA FINAL LIMPIA — sin un solo nulo en variables del modelo
