...
from "patient" p where (p.age = '> 89' or (p.age ~ '^\d+$' and cast(p.age as integer) >= 18)) and p.unitdischargeoffset > 2880;
...
from cohorte_base_eicu_v4l c left join noradrenalina_eicu n on c.patientunitstayid = n.patientunitstayid where n.inicio_norad_min is null or n.inicio_norad_min >= 0;
...
from cohorte_norad_eicu_v4l where horas_hasta_norad is null or horas_hasta_norad >= 12;

-- 4. LABORATORIO (ventana 0-12h)
--    Para v4l: lactato_max, bicarbonato_min, bilirrubina_media, glucemia_min,
--              pao2_min (pf) + auxiliares SOFA (bilirrubina_max, plaquetas_min, creatinina_max)
drop table if exists variables_lab_eicu_v4l; create table variables_lab_eicu_v4l as
select c.patientunitstayid,
    ...
    avg(case when l.labname = 'total bilirubin' then l.labresult end) as bilirrubina_media,
...
from dataset_modelo_eicu_v4l c left join "lab" l on c.patientunitstayid = l.patientunitstayid and l.labresult is not null
    and l.labresultoffset between 0 and 720
    and l.labname in ('lactate','bicarbonate','total bilirubin','glucose','paO2','platelets x 1000','creatinine')
group by c.patientunitstayid;


-- 5. VITALES (ventana 0-12h)
drop table if exists variables_vitales_eicu_v4l;
create table variables_vitales_eicu_v4l as with periodic as (select c.patientunitstayid,
    ...
    from dataset_modelo_eicu_v4l c left join "vitalPeriodic" vp on c.patientunitstayid = vp.patientunitstayid and vp.observationoffset between 0 and 720
    group by c.patientunitstayid),
	
aperiodic as (select c.patientunitstayid, min(va.noninvasivemean) as map_nibp_min from dataset_modelo_eicu_v4l c
    left join "vitalAperiodic" va on c.patientunitstayid = va.patientunitstayid and va.observationoffset between 0 and 720
    group by c.patientunitstayid),
	
temp_nurse as (select c.patientunitstayid, min(case
    ...
    left join "nurseCharting" nc on c.patientunitstayid = nc.patientunitstayid and nc.nursingchartoffset between 0 and 720
        and nc.nursingchartcelltypevalname in ('Temperature (C)', 'Temperature (F)') group by c.patientunitstayid)
		
select
    p.patientunitstayid, p.hr_media, p.rr_max, p.spo2_min,
    coalesce(p.temp_min_periodic, t.temp_min_nurse) as temp_min,
    coalesce(p.map_art_min, a.map_nibp_min)         as map_min
from periodic p left join aperiodic  a on p.patientunitstayid = a.patientunitstayid left join temp_nurse t on p.patientunitstayid = t.patientunitstayid;


-- 6. FiO2 NORMALIZADO (ventana 0-12h) — solo para pf_min
drop table if exists variables_fio2_eicu_v4l;
create table variables_fio2_eicu_v4l as
with raw as (select c.patientunitstayid, case when rc.respchartvalue ~ '^\d+(\.\d+)?$' then cast(rc.respchartvalue as numeric) end as fio2_raw
    from dataset_modelo_eicu_v4l c
    left join "respiratoryCharting" rc on c.patientunitstayid = rc.patientunitstayid and rc.respchartoffset between 0 and 720
        and rc.respchartvaluelabel in ('FiO2', 'FIO2 (%)', 'Set Fraction of Inspired Oxygen (FIO2)')),
		
normalizado as (select patientunitstayid,
        case
...
-- 7. GCS (ventana 0-12h) — gcs_min va al SELECT final en v4l
...
-- 8. DIURESIS normalizada por peso (ventana 0-12h)
...
from dataset_modelo_eicu_v4l c left join "intakeOutput" io on c.patientunitstayid = io.patientunitstayid and io.intakeoutputoffset between 0 and 720
group by c.patientunitstayid, c.peso_kg;


-- 9. VENTILACIÓN INVASIVA (ventana 0-12h)
drop table if exists variables_ventilacion_eicu_v4l;
create table variables_ventilacion_eicu_v4l as
select c.patientunitstayid,
    case when exists (
        select 1 from "respiratoryCare" rc
        where rc.patientunitstayid = c.patientunitstayid
          and rc.respcarestatusoffset between 0 and 720
          and rc.airwaytype in ('Oral ETT', 'Nasal ETT', 'Tracheostomy',
                                'Double-Lumen Tube', 'Cricothyrotomy')
    ) then 1 else 0 end as ventilacion_invasiva_12h
from dataset_modelo_eicu_v4l c;


-- 10. SOFA (ventana 0-12h)
drop table if exists variables_sofa_eicu_v4l;
create table variables_sofa_eicu_v4l as
with vasopresores as (select c.patientunitstayid,
    ...
    from dataset_modelo_eicu_v4l c left join "infusionDrug" id on c.patientunitstayid = id.patientunitstayid
        and id.infusionoffset between 0 and 720 and lower(id.drugname) not like '%volume%'
    group by c.patientunitstayid),
...
-- 11. TABLA FINAL — solo las 14 variables del modelo + identificadores
drop table if exists dataset_final_eicu_v4l;
create table dataset_final_eicu_v4l as
select c.uniquepid as subject_id, c.patientunitstayid as stay_id, c.anchor_age,c.gender,c.peso_kg,c.contador_estancia_uci,c.horas_hasta_norad,c.etiqueta_norad_12_48,
    -- 14 variables del modelo v4l
    vt.map_min,vt.hr_media,
    case
        when l.pao2_min is not null and f.fio2_max is not null and f.fio2_max > 0
        then l.pao2_min / (f.fio2_max / 100.0)
        else null
    end as pf_min, vt.spo2_min, vt.rr_max,
    coalesce(vm.ventilacion_invasiva_12h, 0) as ventilacion_invasiva_12h, d.diuresis_ml_kg_12h,l.lactato_max,l.bicarbonato_min,l.bilirrubina_media,g.gcs_min,l.glucemia_min,vt.temp_min,so.sofa_max
from dataset_modelo_eicu_v4l c
left join variables_lab_eicu_v4l l  on c.patientunitstayid = l.patientunitstayid
left join variables_vitales_eicu_v4l vt on c.patientunitstayid = vt.patientunitstayid
left join variables_fio2_eicu_v4l f on c.patientunitstayid = f.patientunitstayid
...
left join variables_gcs_eicu_v4l g on c.patientunitstayid = g.patientunitstayid
left join variables_ventilacion_eicu_v4l vm on c.patientunitstayid = vm.patientunitstayid
...
where anchor_age              is not null
  ...
  and ventilacion_invasiva_12h is not null
