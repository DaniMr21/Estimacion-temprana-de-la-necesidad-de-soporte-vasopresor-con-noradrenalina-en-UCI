...
-- Cohorte base v3.2: adultos con estancia UCI > 24h  -----DIF FUNBDAMENTAL CON V3
...
where p.anchor_age >= 18  --fuera pediátricos
  and extract(epoch from (i.outtime - i.intime)) / 3600.0 > 24;  --fuera estancias cortas (postop, traslados rápidos); evita falsos negativos en ventana 6-24h
...
drop table if exists public.dataset_final_v3_2_clean;

create table public.dataset_final_v3_2_clean as
select
    subject_id,
    hadm_id,
    stay_id,
...
select * from public.dataset_final_v3_2_clean;

select count(*) from public.dataset_final_v3_2_clean;
