...
def preparar(df):
    ...
    fuera = [
        "stay_id", "subject_id", "hadm_id", "intime", "outtime", "rn",
        "horas_uci", "inicio_noradrenalina", "horas_hasta_norad"
    ]

    x = df.drop(columns=fuera + [target]).copy()
    y = df[target].copy()

    # no meter variables derivadas porque no mejoran el AUC
...
def modelo_lightgbm(x_train, x_test, y_train, y_test):
    pipeline = Pipeline([
        ...
        ("modelo", LGBMClassifier(n_estimators=300, max_depth=2, learning_rate=0.03, subsample=0.8, colsample_bytree=0.6, random_state=42, verbosity=-1))])
