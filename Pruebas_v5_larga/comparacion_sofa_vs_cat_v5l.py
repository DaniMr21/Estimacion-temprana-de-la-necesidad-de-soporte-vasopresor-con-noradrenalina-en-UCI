"""
Comparación SOFA solo vs modelo CatBoost final — v4l (12-48h)

Compara:
  A) SOFA como predictor único (score clínico estándar)
  B) Modelo CatBoost con el set final de 14 variables

Salidas:
  - tablas/comparacion_sofa_vs_cat_v4l.csv
"""
...
def main():
    print("=" * 65)
    print("COMPARACIÓN SOFA vs CatBoost — v4l (12-48h)")
    print("=" * 65)
    ...
    print(f"\n{'='*65}")
    print(f"RESULTADOS FINALES — v4l (12-48h)")
    print(f"{'='*65}")
    print(f"  SOFA solo (1 var)       : AUC = {auc_s_m:.4f} ± {auc_s_s:.4f}")
    print(f"  CatBoost (14 vars)      : AUC = {auc_c_m:.4f} ± {auc_c_s:.4f}")
    ...
    ruta = os.path.join(CARPETA_TABLAS, 'comparacion_sofa_vs_cat_v4l.csv')
