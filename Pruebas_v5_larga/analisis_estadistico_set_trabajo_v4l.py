"""
Análisis estadístico completo — v4l SET DE TRABAJO (15 variables).
Ventana: 0-12h observación / 12-48h predicción.

Set de trabajo:
  - 13 del backward elimination
  - + ph_min        (preferencia clínica sobre bicarbonato_min)
  - + gcs_min       (borderline en backward, criterio clínico)

Ejecuta en secuencia:
  1. Significancia univariante (Mann-Whitney / Chi² + BH-FDR)
  2. Regresión logística multivariante (statsmodels + BH-FDR)
  3. Permutation importance sobre CatBoost (modelo ganador para v4l)

Salidas (nombres únicos para no pisar análisis anteriores):
  - tablas/univariante_set_trabajo_v4l.csv
  - tablas/multivariante_set_trabajo_v4l.csv
  - tablas/permutation_importance_set_trabajo_v4l.csv
  - figuras/permutation_importance_set_trabajo_v4l.png
"""
...
# Set de trabajo: 13 del backward + ph_min + gcs_min
VARIABLES = [
    # Hemodinámica
    'map_min', 'hr_media',
    # Respiratorio
    ...
    # Renal
    'diuresis_ml_kg_12h',
    # Ácido-base / perfusión
    'lactato_max',
    'bicarbonato_min',
    'ph_min',                    # ← AÑADIDA por preferencia clínica
    # Hepático
    'bilirrubina_media',
    # Neurológico
    'gcs_min',                   # ← AÑADIDA (borderline backward)
    # Metabólico
    'glucemia_min',
    # Vital
    'temp_min',
...
VARS_BINARIAS  = ['ventilacion_invasiva_12h']
...
# ── CARGA ─────────────────────────────────────────────────────────────────────
print("=" * 68)
print("ANÁLISIS ESTADÍSTICO SET TRABAJO — v4l (15 vars, 12-48h)")
print("=" * 68)

df_full = pd.read_csv(RUTA_CSV)
df_full = df_full.dropna(subset=['pf_max'])

# Para univariante y LR: 1ª estancia por paciente
df_1 = (df_full.sort_values(['subject_id', 'contador_estancia_uci'])
               .drop_duplicates('subject_id', keep='first')
               .reset_index(drop=True))
print(f"\nDataset (1ª estancia/paciente): N={len(df_1)} | "
      f"Positivos={df_1[ETIQUETA].sum()} ({100*df_1[ETIQUETA].mean():.2f}%)")

# Para PI: dataset completo
...
for var in VARS_CONTINUAS:
    ...
    if len(pos)<2 or len(neg)<2:
        filas.append({'variable':var,'test':'Mann-Whitney U','mediana_pos':np.nan,
                      'mediana_neg':np.nan,'p_valor':np.nan})
        continue
    ...
    filas.append({'variable':var,'test':'Mann-Whitney U',
                  ...
                  'mediana_neg':round(neg.median(),4),'p_valor':p})

for var in VARS_BINARIAS:
    tabla = pd.crosstab(X1[var], y1)
    if tabla.shape[0]<2 or tabla.shape[1]<2:
        filas.append({'variable':var,'test':'Chi-cuadrado','mediana_pos':np.nan,
                      'mediana_neg':np.nan,'p_valor':np.nan})
        continue
    chi2, p, _, _ = chi2_contingency(tabla)
    filas.append({'variable':var,'test':'Chi-cuadrado',
                  'mediana_pos':round(X1.loc[y1==1,var].mean(),4),
                  'mediana_neg':round(X1.loc[y1==0,var].mean(),4),'p_valor':p})

uni = pd.DataFrame(filas)
mask = uni['p_valor'].notna()
rechaza, p_corr, _, _ = multipletests(uni.loc[mask,'p_valor'], method='fdr_bh', alpha=0.05)
uni['p_valor_BH'] = np.nan; uni['sig_BH'] = False
uni.loc[mask,'p_valor_BH'] = p_corr; uni.loc[mask,'sig_BH'] = rechaza
...
uni.to_csv(os.path.join(CARPETA_TABLAS,'univariante_set_trabajo_v4l.csv'), index=False)
...
X1_std = pd.DataFrame(StandardScaler().fit_transform(X1_imp),
                      columns=X1_imp.columns, index=X1_imp.index)
...
ic = modelo_lr.conf_int(); ic.columns = ['IC_inf','IC_sup']
multi = pd.DataFrame({
...
rechaza2, p_corr2, _, _ = multipletests(multi['p_valor'], method='fdr_bh', alpha=0.05)
multi['p_valor_BH'] = p_corr2; multi['sig_BH'] = rechaza2
multi = multi.sort_values('p_valor').reset_index(drop=True)

mp = multi[['variable','coef','OR','OR_IC_inf','OR_IC_sup','p_valor','p_valor_BH','sig_BH']].copy()
...
multi.to_csv(os.path.join(CARPETA_TABLAS,'multivariante_set_trabajo_v4l.csv'), index=False)


# ═══════════════════════════════════════════════════════════════════════════════
# 3. PERMUTATION IMPORTANCE — CatBoost (modelo ganador v4l)
# ═══════════════════════════════════════════════════════════════════════════════
print("\n" + "─"*68)
print("[3/3] PERMUTATION IMPORTANCE — CatBoost")
print("─"*68)
...
for nf, (idx_tr, idx_te) in enumerate(
        cv_ext.split(X_full, y_full, groups=pid_full), start=1):
    ...
    gs = GridSearchCV(pipeline_pi, espacio_pi, cv=cv_int,
    ...
    res = permutation_importance(gs.best_estimator_, x_te, y_te,
...
auc_pi = np.mean(aucs_pi); std_pi = np.std(aucs_pi)
print(f"\n  AUC medio CatBoost (15 vars): {auc_pi:.4f} ± {std_pi:.4f}  "
...
pi = pd.DataFrame(filas_pi).sort_values('caida_AUC_pp', ascending=False).reset_index(drop=True)

tp = pi.copy()
for c in ['caida_AUC_pp','caida_AUC_desv','IC_inf','IC_sup','p_valor_empirico']:
    tp[c] = tp[c].round(4)
...
colores = ['#2ca02c' if r else '#d62728' for r in df_plot['IC_excluye_cero']]
fig, ax = plt.subplots(figsize=(10,7))
pos = np.arange(len(df_plot))
ax.barh(pos, df_plot['caida_AUC_pp'], xerr=df_plot['caida_AUC_desv']*100,
        color=colores, edgecolor='black', linewidth=0.5, alpha=0.85, capsize=3)
ax.set_yticks(pos); ax.set_yticklabels(df_plot['variable'])
ax.set_xlabel('Caída de AUC (pp) al permutar la variable')
ax.set_title(f'Permutation Importance — CatBoost SET TRABAJO v4l (15 vars)\n'
...
plt.tight_layout()
ruta_fig = os.path.join(CARPETA_FIGS, 'permutation_importance_set_trabajo_v4l.png')
plt.savefig(ruta_fig, dpi=150, bbox_inches='tight'); plt.close()

pi.to_csv(os.path.join(CARPETA_TABLAS,'permutation_importance_set_trabajo_v4l.csv'), index=False)
print(f"\n  Figura: {ruta_fig}")


# ═══════════════════════════════════════════════════════════════════════════════
# COMPARACIÓN ESPECÍFICA pH vs BICARBONATO
# ═══════════════════════════════════════════════════════════════════════════════
print("\n" + "═"*68)
print("COMPARACIÓN pH_MIN vs BICARBONATO_MIN")
print("═"*68)

for var in ['ph_min', 'bicarbonato_min']:
    u_p  = uni.loc[uni['variable']==var, 'p_valor_BH'].iloc[0]
    u_s  = uni.loc[uni['variable']==var, 'sig_BH'].iloc[0]
    m_p  = multi.loc[multi['variable']==var, 'p_valor_BH'].iloc[0]
    m_s  = multi.loc[multi['variable']==var, 'sig_BH'].iloc[0]
    m_or = multi.loc[multi['variable']==var, 'OR'].iloc[0]
    pi_v = pi.loc[pi['variable']==var, 'caida_AUC_pp'].iloc[0]
    pi_s = pi.loc[pi['variable']==var, 'IC_excluye_cero'].iloc[0]
    print(f"\n  {var}:")
    print(f"    Univariante BH : p={u_p:.4f}  sig={u_s}")
    print(f"    LR multi BH    : p={m_p:.4f}  sig={m_s}  OR={m_or}")
    print(f"    PI CAT         : caida={pi_v:.3f}pp  IC excluye 0={pi_s}")
