...
print("=" * 70)
print("CORRELACIONES — v4l SET REDUCIDO")
...
print(f"    Positivos       : {df['etiqueta_norad_12_48'].sum()} "
      f"({100 * df['etiqueta_norad_12_48'].mean():.2f}%)")
print()
...
ax.set_title(
    'Matriz de correlación de Spearman — v4l reducido',
    fontsize=13,
    pad=15
)
...
ruta_heatmap = os.path.join(
    CARPETA_FIGURAS,
    'correlacion_spearman_v4l_reducido.png'
)
...
ruta_matriz = os.path.join(
    CARPETA_TABLAS,
    'matriz_spearman_v4l_reducido.csv'
)
...
ruta_pares = os.path.join(
    CARPETA_TABLAS,
    'pares_correlacion_v4l_reducido.csv'
)
...
ruta_asoc = os.path.join(
    CARPETA_TABLAS,
    'asociaciones_binarias_v4l_reducido.csv'
)
...
ax.set_title(
    'Dendrograma — v4l reducido\nDistancia = 1 - |rho Spearman|',
    fontsize=12,
    pad=10
)

ax.set_xlabel('Distancia (1 - |rho|)')
ax.axvline(x=0.3, color='red', linestyle='--', linewidth=1, label='|rho| = 0.7')
ax.axvline(x=0.5, color='orange', linestyle='--', linewidth=1, label='|rho| = 0.5')
...
print("=" * 70)
print("RESUMEN EJECUTIVO — v4l REDUCIDO")
print("=" * 70)
