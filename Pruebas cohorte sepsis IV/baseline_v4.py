...
def cargar_datos():
    ...
    df = pd.read_csv(ruta)
    # 1 NaN aislado en pf_max (caso único por el winsorizado); los modelos sin
    # imputación lo rechazarían. El resto de NaN legítimos están en
    # horas_hasta_norad (controles) y esa variable NO se usa como predictora.
...
def preparar(df):
    ...
    variables_predictoras = [
        ...
        # Constantes vitales (chartevents, también ventana 0-6h)
    ...
    #1 si el paciente inició noradrenalina entre las horas 6 y 24 desde el ingreso, 0 en caso contrario.
    etiqueta = df['etiqueta_norad_6_24'].copy()
 
    # subject_id se extrae aparte: se usará como "grupo" en el CV para asegurar que un mismo paciente nunca aparezca simultáneamente en train y test.
...
def validacion_cruzada_anidada(nombre_modelo, pipeline, espacio_hiperparametros,
    ...
    # Iteramos sobre los 5 folds del CV externo
    for num_fold, (indices_train, indices_test) in enumerate(
        ...
        busqueda = GridSearchCV(
            ...
            scoring='roc_auc',
            n_jobs=n_jobs,
            refit=True  # reentrena el mejor modelo con todo el train del fold
        )
        busqueda.fit(x_train, y_train, groups=pacientes_train)
 
        # Evaluación en el test del fold externo (datos nunca vistos)
    ...
    # Agregación de resultados: media y desviación típica sobre los 5 folds
...
def main():
    ...
    resultados['CAT'] = validacion_cruzada_anidada(
        ...
        espacio_catboost,
        predictores, etiqueta, paciente_id,
        n_jobs=1  # evita conflicto con la paralelización interna de CatBoost
    )
