...
VARIABLES  = ['pf_min', 'map_min', 'diuresis_ml_kg_3h', 'sofa_max']
N_SPLITS    = 5
RANDOM_SEED = 42
N_BINS_CAL  = 10
...
resultados = [
    ...
    metricas_resumen(prob_platt,   y, 'XGB Platt'),
    metricas_resumen(prob_isoton,  y, 'XGB Isotónica'),
]
...
for _, fila in df_resultados.iterrows():
    marca = " ←" if fila['nombre'] == mejor_xgb['nombre'] else ""
    print(f"  {fila['nombre']:<22} {fila['auc']:>7.4f} {fila['bss']:>+8.4f} {fila['ece']:>8.4f}{marca}")
...
colores    = {'XGB sin calibrar': '#ff7f0e', 'XGB Platt': '#1f77b4', 'XGB Isotónica': '#2ca02c'}
probs_dict = {'XGB sin calibrar': prob_sin_cal, 'XGB Platt': prob_platt, 'XGB Isotónica': prob_isoton}
...
ax2.axhline(y=CAT_CORTO_REF['ece'], color='purple', lw=1.5, linestyle=':',
            label=f"CAT Corto ECE={CAT_CORTO_REF['ece']:.4f}")
...
for barra, valor in zip(barras, eces):
    ax2.text(barra.get_x() + barra.get_width() / 2, barra.get_height() + 0.0005,
...
barras3  = ax3.bar(nombres, bss_vals, color=[colores[n] for n in nombres],
                   alpha=0.85, edgecolor='black')
ax3.axhline(y=CAT_CORTO_REF['bss'], color='purple', lw=1.5, linestyle=':',
            label=f"CAT Corto BSS={CAT_CORTO_REF['bss']:+.4f}")
