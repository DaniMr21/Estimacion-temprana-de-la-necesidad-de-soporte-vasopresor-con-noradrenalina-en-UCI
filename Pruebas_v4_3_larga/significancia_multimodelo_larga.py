"""
Significancia estadística multimodelo — set reducido v4l (26 variables).

Integra dos niveles de análisis de significancia:

[1] SIGNIFICANCIA UNIVARIANTE (rápido, ~segundos)
    Contrasta cada variable por separado frente a la etiqueta.
      - Variables continuas : Mann-Whitney U (two-sided)
      - Variables binarias  : Chi-cuadrado
      - Corrección múltiple : FDR Benjamini-Hochberg (q < 0.05)
    Nota: se filtra a la primera estancia por paciente para evitar
    dependencia entre observaciones (mismo criterio que significancia.py).

[2] PERMUTATION IMPORTANCE MULTIMODELO (costoso, horas)
    Extiende el análisis de permutation importance (ya realizado para RF
    en permutation_importance_RF_v4.py) a seis modelos:
      ...
      - CatBoost            (CAT)
      - Naive Bayes         (NB)
    Para cada modelo y variable se calcula la caída de AUC al permutar
    la variable sobre datos de test no vistos (5 folds externos).
    Una variable es significativa si el IC95% de esa caída excluye cero.
    Nota: usa todas las estancias (no filtradas) con agrupamiento por
    paciente en la CV, lo que es coherente con el pipeline de modelado.

[3] TABLA RESUMEN GLOBAL
    Consolida ambos análisis en una única tabla:
      - Columna univariante (Mann-Whitney / Chi²)
      - Una columna por modelo (permutation importance)
      - n_modelos_significativos y categoría de consenso
    Ordenada por n_modelos_significativos desc, luego por p univariante.

Metodología permutation importance:
  - CV externa: 5 folds, StratifiedGroupKFold
  - CV interna: 3 folds, búsqueda de hiperparámetros
  - Permutaciones por variable y fold: N_REPETICIONES_PERMUTACION (50)
  - Métrica: AUC-ROC
  - Grids REDUCIDOS (centrados en best_params del baseline conocido)

Salidas:
  - tablas/significancia_univariante_multimodelo_v4.csv
  - tablas/permutation_importance_multimodelo_v4.csv
  - tablas/tabla_resumen_global_significancia_v4.csv
  - figuras/significancia_global_v4.png
"""
